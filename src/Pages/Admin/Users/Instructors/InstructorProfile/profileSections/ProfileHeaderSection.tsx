...
const ProfileHeaderSection = ({ instructor, onEdit }: Props) => {
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-[20px] font-semibold text-[#333333] dark:text-white">{instructor.name}</h1>
              <span className={`px-3 py-1 rounded-full text-[12px] font-regular ${
                instructor.status === 'Active' ? 'bg-[#047C2E]/10 text-[#047C2E] dark:bg-green-500/10 dark:text-green-400' :
            ...
            <p className="md:text-[16px] text-sm text-[#626262] dark:text-[#A3A3A3] mt-1">ID: {instructor.instructorId}</p>
          </div>
        </div>
          ...
          <div className="bg-white dark:bg-[#2A2A2A] rounded-[24px] w-full max-w-md overflow-hidden shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
            <div className="p-8 pb-6 flex flex-col items-center text-center">
              ...
              <p className="text-[14px] text-[#626262] dark:text-[#A3A3A3] leading-relaxed mb-8">
                Are you sure you want to deactivate <span className="font-bold text-[#0B1C30] dark:text-white">{instructor.name}'s</span> account? This will restrict their access to all live classes and coursework. This action can be undone by an administrator later.
              </p>
              ...
              <div className="w-full bg-[#FFFBF8] dark:bg-[#333] border border-[#FFF0E5] dark:border-[#3B3B3B] rounded-[16px] p-4 flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  <img src={instructor.avatar} alt="Profile" className="w-[40px] h-[40px] rounded-full object-cover" />
                  <div className="text-left">
                    <p className="text-[14px] font-semibold text-[#0B1C30] dark:text-white leading-tight">{instructor.name}</p>
                    <p className="text-[12px] text-[#626262] dark:text-[#A3A3A3] mt-0.5">ID: {instructor.instructorId}</p>
                  </div>
                </div>
              ...
              <div className="w-full flex items-center gap-4">
                ...
                <button 
                  onClick={() => {
                    // Logic to handle deactivation goes here
