...
const StudentEditMode = ({ student, onCancel }: Props) => {
  const [attachments, setAttachments] = useState([
    { id: 1, name: 'Tenth_marksheet.pdf', size: '1.2 MB', date: 'Jan 12, 2026' },
    { id: 2, name: 'Twelfth_marksheet.pdf', size: '1.5 MB', date: 'Jan 12, 2026' },
  ]);
    ...
    <div className="w-full space-y-6 animate-in fade-in duration-300">
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-[20px] font-semibold text-[#0B1C30] dark:text-white">{student.name}</h1>
            ...
            <p className="text-[14px] text-[#626262] dark:text-[#A3A3A3] mt-1">ID: {student.id}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative flex items-center">
              ...
              <input
                type="text"
                defaultValue={student.phone?.replace('+91', '') || ''}
      ...
      <div className="boxStyle p-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-[16px] font-semibold text-[#0B1C30] dark:text-white">Academic Information</h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 mb-8">
          <div className="flex flex-col gap-2">
            <label className="text-[12px] text-[#626262] dark:text-[#A3A3A3] font-medium">Enrolled Course</label>
            <input
              type="text"
          ...
          <div className="flex flex-col gap-2">
            <label className="text-[12px] text-[#626262] dark:text-[#A3A3A3] font-medium">Batches</label>
            <input
              type="text"
              defaultValue={student.batches.join(', ')}
              className="w-full px-4 py-3 border border-[#E5E7EB] dark:border-[#3B3B3B] rounded-[12px] bg-white dark:bg-[#242424] text-[14px] text-[#0B1C30] dark:text-white focus:outline-none focus:border-[#F6810C]"
            />
          </div>
        </div>
            ...
            <div className="mt-5 space-y-4">
              {attachments.map((file) => {
                ...
                return (
                  <div
                    key={file.id}
                    className="group flex items-center justify-between transition-all duration-300 cursor-pointer sm:rounded-3xl rounded-[20px] gap-10 py-1 md:pr-5 pr-3 pl-1 border border-[#F2EEF4] dark:border-[#3B3B3B] animate-in slide-in-from-top-2"
                  >
