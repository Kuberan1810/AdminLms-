...
import {Plus,Trash2, Search,ArrowLeft, Download} from "lucide-react";
      ...
      <div className="boxStyle flex flex-col sm:flex-row items-start sm:items-center justify-between rounded-xl gap-3">
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <div className="flex flex-col justify-center items-start">
            <h2 className="lg:text-2xl text-lg  font-semibold truncate text-[#333] ">{batchName}</h2>
            <p className="lg:text-lg text-base font-medium truncate text-[#808080] ">{courseName}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 my-6">
        {stats.map((s) => (
          <div key={s.label} className="boxStyle">
            <p className="text-base lg:text-xl text-[#333] font-medium">{s.label}</p>
            <h2 className={`lg:text-2xl text-xl  font-semibold mt-2 ${s.color ?? ""}`}>
              {s.value}
            </h2>
            <p className="text-sm lg:text-base  text-gray-400 mt-1">{s.sub}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="lg:col-span-2 boxStyle ">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
            <h3 className="font-semibold text-lg lg:text-xl text-[#333]">Curriculum</h3>
          ...
          <div className="space-y-5">
            {filteredModules.map((m, index) => (
              <div key={m.id} className="border border-[#F2EEF4] rounded-xl">
                ...
                <div className="flex justify-between items-center px-4 py-5 bg-white rounded-xl">
                  <div className="space-x-5">
                    <span className="text-sm lg:text-lg font-medium">
                      Module {index + 1}: {m.title}
                    </span>
                    <span className="bg-[#FFEDDE] ml-3 text-[#F67300] text-xs lg:text-sm font-medium px-4 py-2 rounded-full">
                      {m.status}
                    </span>
                  </div>
                ...
                {openModule === m.id && (
                  <div className="p-4 space-y-2">
                    <div className="flex justify-between">
                      <span className="text-sm lg:text-base font-medium">Chapters:</span>
                      <button
                        onClick={() => setShowChapterModal(m.id)}
                        className="bg-gray-100 p-2 rounded-xl cursor-pointer hover:bg-gray-200"
                      >
                        <Plus size={16} />
                      </button>
                    </div>
                    ...
                    {m.chapters.map((c) => (
                      <div key={c.id}>
                        <div
                          className="flex justify-between border border-[#F2EEF4] rounded-lg p-2.5 text-sm text-[#333]"
                        >
                          <div onClick={() => {
                    ...
                    <div className="mt-4">
                      <div className="flex justify-between items-center mb-3">
                        <h4 className="text-sm lg:text-base font-medium">Assignments:</h4>
                        <button
                      ...
                      <div className="space-y-2">
                        {modulesState.byId[m.id].assignmentIds.map((assignmentId) => {
                          ...
                          return (
                            <div key={assignmentId} className="flex justify-between items-center text-sm border border-[#F2EEF4] p-2.5 rounded-lg">
                              <div
                                className="cursor-pointer flex-1"
                                onClick={() => navigate("/instructor/assignment-details", { state: { ...assignment, isEdit: true, moduleId: m.id } })}
                              >
                                <span className="text-sm lg:text-base">{assignment.title}</span>
                              </div>
                    ...
                    <div className="mt-4">
                      <div className="flex justify-between items-center mb-3">
                        <h4 className="text-sm lg:text-base font-medium">Tests:</h4>
                        <button
                      ...
                      <div className="space-y-2">
                        {modulesState.byId[m.id].testIds.map((testId) => {
                          ...
                          return (
                            <div key={testId} className="flex justify-between items-center text-sm border border-[#F2EEF4] p-2.5 rounded-lg">
                              <div className="flex-1">
                                <span className=" cursor-pointer text-sm lg:text-base" onClick={() => handleTestRedirect(testId)}>{test.name}</span>
                              </div>
                              <div className="flex items-center gap-3">
                                <span className="text-gray-400 text-sm">Date: {test.date}</span>
        ...
        <div className="boxStyle h-fit">
          <h3 className="font-semibold text-lg lg:text-xl mb-5 text-[#333]">Recent activity</h3>

          {activities.map((text, i) => (
            <div
              key={i}
              className="flex justify-between items-center boxStyle mb-3 bg-[#fafafa]!"
            >
              <div className="flex flex-col">
                <span className="text-sm lg:text-base font-medium text-[#333]">{text}</span>
                <span className="text-sm lg:text-base text-[#808080]">2 min ago</span>
              </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-[320px] shadow-xl">
            ...
            <input
              ...
              className="border border-[#F2EEF4] p-2 rounded w-full mb-4 text-[#626262] "
            />
            ...
            <div className="relative mb-4">
              <input
                ...
                className="border border-[#F2EEF4] p-2 pr-10 rounded w-full text-[#626262] appearance-none"
              />
            ...
            <div className="flex justify-end gap-5">
              <button onClick={() => setShowAssignmentModal(null)} className="cursor-pointer hover:bg-gray-100 px-4 rounded-lg">Cancel</button>
              <button
      ...
      {showTestModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-[320px] shadow-xl">
            ...
            <div className="flex justify-end gap-5">
              <button onClick={() => setShowTestModal(null)} className="cursor-pointer hover:bg-gray-100 px-4 rounded-lg">Cancel</button>
              <button
          ...
          <div className="bg-white rounded-2xl p-6 w-[320px] shadow-xl">
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className="px-4 py-2 border border-[#F2EEF4] rounded-lg cursor-pointer"
              >
                Cancel
              </button>
